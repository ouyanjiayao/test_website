...
cursor.execute("SELECT * FROM `tbl_goods_copy` where youzan_image_ids =''")
...
for row in rows:
    ...
    for img in item['item_imgs']:
        cursor.execute('select * from tbl_upload_file_copy where youzan_id = %s', (img['id']))
        ...
        if not exists:
            ...
            cursor.execute(
                'insert into tbl_upload_file_copy(dir_id,name,url,size,created_time,type,is_delete,version,youzan_id,youzan_version,youzan_syn_time,youzan_syn_state) value(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',
                insert)
    ...
    cursor.execute('update tbl_goods_copy set youzan_image_ids = %s where id = %s',(youzan_image_ids,row['id']))
    cursor.execute('update tbl_goods_copy set images = %s where id = %s', (str(images), row['id']))
    cursor.execute('update tbl_goods_copy set first_image = %s where id = %s', (str(images[0]), row['id']))
