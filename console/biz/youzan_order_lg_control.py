...
class OrderPrint:
    ...
    def __init__(self,type,offline_id):
        ...
        device_id = None
        if type == 1:
            if offline_id == 'lg' or offline_id == 'hr':
                device_id = default_config['print']['third_device_id']
            ...
            if offline_id == 'lg' or offline_id == 'hr':
                device_id = default_config['print']['forth_device_id']
        ...
        if offline_id== 'lg':
            self.subject = default_config['offline_info']['lg_subject']
            self.offline_id = default_config['offline_info']['lg_offline_id']
        elif offline_id== 'hr':
            self.subject = default_config['offline_info']['hr_subject']
            self.offline_id = default_config['offline_info']['hr_offline_id']
        else:
            self.subject = default_config['offline_info']['st_subject']
            self.offline_id = default_config['offline_info']['st_offline_id']
    ...
    def execute(self, limit):
        ...
        self.cursor = cursor
        cursor.execute('select * from tbl_youzan_order where '+print_state_column+' = 0 and order_state > 1 and offline_id = "'+self.offline_id+'" and order_num is not null order by id asc limit %s', (limit))
    ...
    def generate_print_content(self,type,order_data,row):
        title = None
        if type == 1:
            title = self.subject+'总台单'
        elif type == 2:
            title = self.subject+'分控单'
        elif type == 3:
            title = self.subject+'配送单'
        elif type == 4:
            title = self.subject+'采购单'
