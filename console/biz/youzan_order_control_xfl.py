...
class OrderPrint:
    ...
    def __init__(self,type):
        ...
        device_id = None
        if type == 1:
            device_id = default_config['print']['cs_device_id']
        elif type == 2:
            device_id = default_config['print']['cs_device_id']
    ...
    def execute(self, limit):
        ...
        self.cursor = cursor
        cursor.execute('select * from tbl_youzan_order_xfl where '+print_state_column+' = 0 and order_state > 1 and order_num is not null order by id asc limit %s', (limit))
        ...
        for row in rows:
            ...
            time.sleep(1)
        if len(updates) > 0:
            cursor.executemany('update tbl_youzan_order_xfl set '+print_state_column+' = %s,'+print_time_column+' = %s where id = %s', updates)
...
class OrderControl:
    ...
    def execute(self,limit):
        ...
        cursor = conn.cursor(cursor=pymysql.cursors.DictCursor)
        self.cursor = cursor
        cursor.execute('select id from tbl_youzan_order_xfl where order_state = 2 and order_num is null order by id asc limit %s',(limit))
        rows = cursor.fetchall()
        updates = []
        for row in rows:
            state = 2
        ...
        if len(updates) > 0:
            cursor.executemany("update tbl_youzan_order_xfl set order_num = %s where id = %s",updates)
