...
class OrderPrint:
    ...
    def generate_print_content(self,type,order_data,row):
        ...
        for detail in order_data['details']:
            ...
            content += '<tr><td>{title} x{num}</td><td> </td><td>￥{price}</td></tr><tr><td>{detail_desc}</td></tr>'.format(**detail)
        ...
        if len(order_data['tc_config'])>0:
            ...
            for tc_config in order_data['tc_config']:
                ...
                for tc_config_content in tc_config['tc_config']:
                    cp_item_content = '<FB>{cp_name}</FB> x{count}\r\n\r\n'
                    dp_item_contents = []
                    for dp_config_content in tc_config_content['dp_config']:
                        if 'sku_id' in dp_config_content.keys() and dp_config_content['sku_id'] != tc_config['tc_sku_id']:
                            continue
                        dp_item_content = '- {dp_name} x{count}'
                        if dp_config_content['desc']:
                            dp_item_content += '\r\n{dp_desc}'
                        dp_item_contents.append(dp_item_content.format(**{'dp_name':dp_config_content['dp_name'],'count':dp_config_content['count'],'dp_desc':dp_config_content['desc']}))
                    dp_item_contents = '\r\n\r\n'.join(dp_item_contents)
                    cp_item_content += dp_item_contents
                    cp_item_contents.append(cp_item_content.format(**{'cp_name':tc_config_content['cp_name'],'count':tc_config_content['count']}))
    ...
    def generate_order_data(self,row):
        ...
        for order in push_content['full_order_info']['orders']:
            ...
            sku_properties_name = []
            cursor.execute('select * from tbl_goods_sku_detail where sku_id = %s', (order['outer_sku_id']))
