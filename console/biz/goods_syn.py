    ...
    if cp_config_rows:
        for cp_config_row in cp_config_rows:
            ...
            if not cp_config_content['attr']:
                ...
                cursor.execute('select * from tbl_goods_sku_system where goods_id = %s and sku_id = %s and type=2',
...
class GoodsSyn:
        ...
        while i < len(images):
            ...
            if image_row:
                if try_count > 10:
                    i += 1
                    try_count = 0
                if not(image_row or image_row['youzan_id']):
                    time.sleep(2)
                    try_count += 1
                else:
    ...
    def execute_delete(self,limit):
        ...
        if rows:
            ...
            if len(log_inserts) > 0:
                cursor.executemany(
                    "insert into tbl_youzan_syn_log(response_content,created_time,type,syn_id,syn_state,api_name) values(%s,%s,%s,%s,%s,%s)",
                    log_inserts)
            cursor.close()