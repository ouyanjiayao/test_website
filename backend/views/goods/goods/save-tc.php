...
Vue.component('goods-dp-config',{
	...
	data:function(){
		var data = {
			selectRows:[],
			listUrl:'<?=Yii::$app->user->checkAccess('cp-config-list-data')?Url::toRoute('cp-config-list-data'):'' ?>',
...
Vue.component('goods-sku-detail',{
	...
	template:`
		...
		<goods-sku-table :attr-assign="attrAssignData" :table-data.sync="skuTableData" :ext-row-props="skuExtRowProp">
    		...
    		<el-table-column width="400px" label="菜品配置">
        		<template slot-scope="scope">
				...
				<div v-for="(dpConfig,index) in scope.row['dpc']" v-key="index" style="margin-bottom:10px;border-bottom: 1px solid #f2f2f2;padding-bottom:10px;">
					<div style="">{{dpConfig['name']}}{{dpConfigAttrName(dpConfig)}}&nbsp;&nbsp;<el-button type="text" @click="removeDpConfig(scope.row,index)">移除</el-button></div>
		<div>成本：￥{{(dpConfig.cost).toFixed(2)}}</div>
				</div>
				</template>
