	...
	methods:{
		...
		confirmSelect:function(row){
			...
			if(row['attr_assign'].length>0)
			{
                ...
                if(row['sku_handle_price'].length>0)
                {
                    var selectAttrs = item['selectAttrs']
                    handlePriceSkuId = [];
                    for(var key in selectAttrs)
                    {
                        if(selectAttrs[key]['attr']['type'] == 2)
                        {
                            handlePriceSkuId.push(selectAttrs[key]['attr']['id']+'_'+selectAttrs[key]['item']['id']);
                        }
                    }
                    handlePriceSkuId = handlePriceSkuId.join('_');
                    if(priceSkuId.length>0){
                        handlePriceSkuId = priceSkuId+':'+handlePriceSkuId;
                    }else{
                        handlePriceSkuId = handlePriceSkuId;
                    }
                    skuHPRow = row['sku_handle_price'].find(function(item){
                         return item['sku_id'] == handlePriceSkuId;
                    });
                    item['handle_price'] = parseFloat(skuHPRow['handle_price']);
                }
			}
...
Vue.component('goods-sku-detail',{
	...
	computed:{
            ...
            return function(row){
                ...
                if(row['dpc'] && row['dpc'].length>0)
                {
                    for(var key in row['dpc'])
                    {
                        if(row['dpc'][key]['price_count_type'] == 1)
                        {
                            jgPrice += row['dpc'][key]['unit'] * row['dpc'][key]['handle_price'];
                        }else if(row['dpc'][key]['price_count_type'] == 2)
                        {
                            jgPrice +=  row['dpc'][key]['handle_price'] /500 * row['dpc'][key]['unit'];
                        }
                    }
                }
                return jgPrice;
		...
		rowCbPrice:function(){
			return function(dpConfig){
			    var unit = dpConfig['unit'] ? dpConfig['unit']:0;
                var cost = dpConfig['cost'] ? dpConfig['cost']:0;
				if(dpConfig['price_count_type'] == 1)
				{
				    return unit*cost;
				}else if(dpConfig['price_count_type'] == 2)
				{
                    return cost/500*unit;
				}
			}
		},
        rowJgPrice:function(){
            return function(dpConfig){
                var unit = dpConfig['unit'] ? dpConfig['unit']:0;
                var handle_price = dpConfig['handle_price'] ? dpConfig['handle_price']:0;
                if(dpConfig['price_count_type'] == 1)
                {
                    return unit * handle_price;
                }else if(dpConfig['price_count_type'] == 2)
                {
                    return handle_price /500 * unit;
                }
            }
        }
    },
	...
	template:`
        ...
        <div>加工价格：￥{{(rowJgPrice(dpConfig)).toFixed(2)}}</div>
				</div>
				</template>
    		...
    		<el-table-column width="200px" label="销售价">
        		<template slot-scope="scope">
            		...
            		<div v-show="attrConfig.auto_create_sale_price">
            		￥ {{(parseFloat((cbPrice(scope.row)*attrConfig['sale_scale']))+parseFloat(jgPrice(scope.row))).toFixed(2)}}
					</div>
        		</template>
            </el-table-column>
...
Vue.$page.mixin.push({
	...
	methods:{
		onSubmit:function(){
			...
			{
				this.$refs['form'].setErrorMessage('至少设置一个规格属性');
				return;
			}
